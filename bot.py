...
# Render (like most PaaS platforms) buffers stdout when it isn't attached to
# a real terminal, so plain print() calls can sit invisible in a buffer for
# a long time — especially from a background thread — instead of showing up
# live in the dashboard logs. Force every print() in this file to flush
# immediately so log output always appears right away.
